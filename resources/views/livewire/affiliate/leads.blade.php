...
<div class="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden">
    ...
    <!-- Enhanced Filters -->
    <div class="relative flex flex-col md:flex-row items-center gap-4 mb-8 bg-white p-4 rounded-3xl border border-slate-200 shadow-sm">
        <div class="relative flex-1 w-full group">
            <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none transition-colors group-focus-within:text-primary-600">
            ...
            <input type="text" wire:model.live="search"
                class="w-full pr-11 pl-4 py-4 bg-slate-50 border-slate-100 rounded-2xl focus:ring-4 focus:ring-primary-100 focus:border-primary-300 transition-all font-bold text-sm text-slate-900 placeholder-slate-400"
        ...
        <div class="flex items-center gap-1.5 bg-slate-100 p-1.5 rounded-2xl border border-slate-200">
            <button wire:click="$set('status_filter', '')"
                class="px-4 py-2.5 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all {{ $status_filter === '' ? 'bg-primary-900 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-200' }}">
                الكل
            </button>
            <button wire:click="$set('status_filter', 'under_review')"
                class="px-4 py-2.5 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all {{ $status_filter === 'under_review' ? 'bg-primary-900 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-200' }}">
                المراجعة
            </button>
            <button wire:click="$set('status_filter', 'sold')"
                class="px-4 py-2.5 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all {{ $status_filter === 'sold' ? 'bg-primary-900 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-200' }}">
                مباع
            </button>
            <button wire:click="$set('status_filter', 'contacting')"
                class="px-4 py-2.5 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all {{ $status_filter === 'contacting' ? 'bg-primary-900 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-200' }}">
                تواصل
            </button>
        </div>
    </div>
    ...
    <div class="relative overflow-x-auto bg-white rounded-3xl border border-slate-200 shadow-sm">
        <table class="w-full text-right border-collapse">
            <thead>
                <tr class="text-slate-400 text-[9px] font-black uppercase tracking-widest bg-slate-50/50">
                    <th class="py-5 pr-6 text-right">اسم العميل</th>
                    <th class="py-5 text-right">القطاع / الأنظمة</th>
                    <th class="py-5 text-right">المدينة</th>
                    <th class="py-5 text-right">معلومات العمولة</th>
                    <th class="py-5 text-right">رقم الهاتف</th>
                    <th class="py-5 text-right">تاريخ الإضافة</th>
                    <th class="py-5 text-right">الحالة</th>
                ...
                <tr class="group hover:bg-slate-50/80 transition-all duration-300">
                    <td class="py-5 pr-6">
                        <div class="flex items-center gap-4">
                            <div class="w-11 h-11 rounded-xl bg-slate-100 flex items-center justify-center text-slate-600 font-extrabold shadow-inner group-hover:bg-primary-900 group-hover:text-white transition-all duration-500">
                                {{ substr($lead->client_name, 0, 1) }}
                            </div>
                            <div>
                                <p class="font-black text-slate-900 text-sm leading-none mb-1.5">{{ $lead->client_name }}</p>
                                @if($lead->company_name)
                                <p class="text-[9px] text-slate-400 font-black uppercase tracking-wider">{{ $lead->company_name }}</p>
                                @endif
                            </div>
                        </div>
                    </td>
                    <td class="py-5">
                        <div class="space-y-2">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-black bg-gray-50 text-gray-500 border border-gray-100">
                                {{ $lead->sector ?? '-' }}
                            </span>
                            @if($lead->recommended_systems)
                            <div class="flex -space-x-2 rtl:space-x-reverse">
                                @foreach($lead->recommended_systems as $sysId)
                                <div class="w-8 h-8 rounded-full bg-white border-2 border-primary-50 p-1 group-hover:rotate-6 transition-transform">
                                    <img src="{{ asset('images/systems/'.$sysId.'.png') }}" class="w-full h-full object-contain" title="{{ $sysId }}" alt="{{ $sysId }}">
                                </div>
                                @endforeach
                            </div>
                            @endif
                    ...
                    <td class="py-5">
                        <span class="text-sm font-black text-primary-900">{{ $lead->city }}</span>
                    </td>
                    <td class="py-5">
                        @php
                        ...
                        <div class="flex flex-col gap-1">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-black text-emerald-600">{{ number_format($totalCommission) }} ريال</span>
                                @elseif($lead->commission_rate)
                                <span class="text-sm font-black text-primary-600">{{ $lead->commission_rate }}%</span>
                                @else
                            ...
                            <div class="bg-emerald-50 px-3 py-1 rounded-xl border border-emerald-100 flex items-center justify-between">
                                <span class="text-[9px] text-emerald-600 font-black">نصيبك</span>
                                <span class="text-[11px] font-black text-emerald-700">{{ number_format($finalShare, 2) }} ر.س</span>
                            </div>
                            @elseif($totalCommission > 0)
                            <div class="bg-primary-50 px-3 py-1 rounded-xl border border-primary-100 flex items-center justify-between">
                                <span class="text-[9px] text-primary-500 font-bold uppercase tracking-tighter">المتوقع</span>
                                <span class="text-[11px] font-black text-primary-900">{{ number_format($finalShare, 2) }} ر.س</span>
                            </div>
                            @endif
                        </div>
                    </td>
                    <td class="py-5">
                        <span class="text-sm font-black text-primary-900 tabular-nums" dir="ltr">{{ $lead->client_phone }}</span>
                    </td>
                    <td class="py-5">
                        <div class="text-sm font-black text-primary-900">{{ $lead->created_at->format('Y/m/d') }}</div>
                        <div class="text-[10px] text-primary-300 font-bold">{{ $lead->created_at->diffForHumans() }}</div>
                    </td>
                    <td class="py-5">
                        @php
                        $statusConfig = [
                        ...
                        @endphp
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black border {{ $config['class'] }}">
                            {{ $config['label'] }}
                        </span>
                    </td>
                    <td class="py-5 pl-4">
